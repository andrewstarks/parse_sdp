...
describe("sdp.parse — media blocks (M5)", function()
  ...
  -- the suffix is /numaddr, never /ttl, and numaddr must be a positive
  -- integer (POS-DIGIT *DIGIT). The check was previously enforced only
  -- at the ST 2110 tier; hoisted to base via valid_connection_address(
  -- tier="base"). This describe covers every IPv6 multicast c= rule
  -- (TTL forbidden + numaddr ABNF + unicast-no-suffix) so the cases
  -- don't drift apart across describes.
  describe("RFC 8866 §5.7 / §9 IPv6 multicast c= rules (base tier)", function()
    ...
    it("accepts IPv6 multicast c= with /numaddr suffix", function()
      local doc = sdp.parse(table.concat({
        "v=0", "o=- 1 1 IN IP4 127.0.0.1", "s=Test",
    ...
    it("rejects IPv6 multicast c= with two suffixes (looks like TTL)", function()
      local doc = sdp.parse(table.concat({
        "v=0", "o=- 1 1 IN IP4 127.0.0.1", "s=Test",
    ...
    it("rejects IPv6 multicast c= with /numaddr=0", function()
      local doc = sdp.parse(make({
        "m=video 5000 RTP/AVP 96",
        "c=IN IP6 ff0e::1/0",
        "a=rtpmap:96 H264/90000",
    ...
    it("rejects IPv6 multicast c= with non-numeric /numaddr", function()
      local doc = sdp.parse(make({
        "m=video 5000 RTP/AVP 96",
        "c=IN IP6 ff02::1/abc",
        "a=rtpmap:96 H264/90000",
  ...
  -- ── Hoisted from ST 2110 tier (base-tier c= validation per RFC 8866 §5.7 / §9) ──
  -- These checks live in valid_connection_address(..., "base") and fire from
  -- both base-tier doc:validate() and the ST 2110 / IPMX tiers. Tests moved
  -- here so they sit alongside the rest of the base c= coverage.

  describe("RFC 8866 §5.7 / §9 multicast TTL range (base tier)", function()
    ...
    it("accepts TTL=64", function()
      local doc = sdp.parse(with_ttl(64))
      assert.is_table(doc)
      local ok, err = doc:validate()
      assert.is_nil(err)
      assert.equal(true, ok)
    ...
    it("accepts TTL=1 (lower boundary)", function()
      local doc = sdp.parse(with_ttl(1))
      assert.is_table(doc)
      local ok, err = doc:validate()
      assert.is_nil(err)
      assert.equal(true, ok)
    ...
    it("accepts TTL=255 (upper boundary)", function()
      local doc = sdp.parse(with_ttl(255))
      assert.is_table(doc)
      local ok, err = doc:validate()
      assert.is_nil(err)
      assert.equal(true, ok)
  ...
  describe("RFC 8866 §9 IPv4 layered multicast numaddr (base tier)", function()
    local PTP = "a=ts-refclk:ptp=IEEE1588-2008:00-11-22-FF-FE-33-44-55:0"
    local VFMTP = "a=fmtp:96 sampling=YCbCr-4:2:2; width=1920; height=1080; exactframerate=25; depth=10; TCS=SDR; colorimetry=BT709; PM=2110GPM; SSN=ST2110-20:2022; TP=2110TPN"
    ...
    it("accepts spec example c=IN IP4 233.252.0.1/127/3", function()
      local doc = sdp.parse(with_c("c=IN IP4 233.252.0.1/127/3"))
      assert.is_table(doc)
      local ok, err = doc:validate()
      assert.is_nil(err)
      assert.equal(true, ok)
    ...
    it("accepts /ttl/numaddr with TTL=0", function()
      local doc = sdp.parse(with_c("c=IN IP4 233.252.0.1/0/3"))
      assert.is_table(doc)
      local ok, err = doc:validate()
      assert.is_nil(err)
      assert.equal(true, ok)
    ...
    it("still accepts plain /ttl form", function()
      local doc = sdp.parse(with_c("c=IN IP4 239.100.0.1/64"))
      assert.is_table(doc)
      local ok, err = doc:validate()
      assert.is_nil(err)
      assert.equal(true, ok)
  ...
  -- RFC 791 / RFC 2460: literal IPv4/IPv6 address syntax. The base-tier
  -- valid_connection_address parses each octet (IPv4) or hextet (IPv6),
  -- not just the first-octet multicast classifier. Originally added under
  -- ST 2110-10 §6.5 (M29 G1); hoisted here because the literal-address
  -- check fires for any c= line, not only RTP media blocks.
  describe("RFC 8866 §5.7 c= IPv4/IPv6 literal address syntax (base tier)", function()
    local function with_c(c_line)
      return table.concat({
        "v=0",
        "o=- 1234567890 1 IN IP4 192.168.1.1",
    ...
    it("rejects IPv4 with only three octets (1.2.3)", function()
      local doc = sdp.parse(with_c("c=IN IP4 1.2.3"))
      assert.is_table(doc)
      local ok, err = doc:validate()
      assert.is_nil(ok)
    ...
    it("rejects IPv4 with octet > 255 (unicast path)", function()
      local doc = sdp.parse(with_c("c=IN IP4 999.0.0.0"))
      assert.is_table(doc)
      local ok, err = doc:validate()
      assert.is_nil(ok)
      assert.matches("IPv4", err.message)
    ...
    it("rejects IPv4 with octet > 255 inside (192.168.999.1)", function()
      local doc = sdp.parse(with_c("c=IN IP4 192.168.999.1"))
      assert.is_table(doc)
      local ok, err = doc:validate()
      assert.is_nil(ok)
      assert.matches("IPv4", err.message)
    ...
    it("rejects IPv4 with five octets (192.168.1.1.5)", function()
      local doc = sdp.parse(with_c("c=IN IP4 192.168.1.1.5"))
      assert.is_table(doc)
      local ok, err = doc:validate()
      assert.is_nil(ok)
      assert.matches("IPv4", err.message)
    ...
    it("rejects IPv6 unicast with non-IPv6 syntax (not-an-ipv6)", function()
      local doc = sdp.parse(with_c("c=IN IP6 not-an-ipv6"))
      assert.is_table(doc)
      local ok, err = doc:validate()
      assert.is_nil(ok)
    ...
    it("rejects IPv6 multicast with garbage tail (ff02::garbage)", function()
      local doc = sdp.parse(with_c("c=IN IP6 ff02::garbage"))
      assert.is_table(doc)
      local ok, err = doc:validate()
      assert.is_nil(ok)
      assert.matches("IPv6", err.message)
    ...
    it("accepts IPv4 max-octet boundary (255.255.255.254)", function()
      local doc = sdp.parse(with_c("c=IN IP4 255.255.255.254"))
      assert.is_table(doc)
      local ok, err = doc:validate()
      assert.is_nil(err)
      assert.equal(true, ok)
    ...
    it("accepts compressed IPv6 unicast (2001:db8::1)", function()
      local doc = sdp.parse(with_c("c=IN IP6 2001:db8::1"))
      assert.is_table(doc)
      local ok, err = doc:validate()
      assert.is_nil(err)
      assert.equal(true, ok)
...
-- ── Serializer: doc:to_sdp() (RFC 8866 §5 field ordering and round-trip) ────
